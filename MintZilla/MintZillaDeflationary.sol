...
// https://www.mintzilla.club/
// Token factory contract(deflationary token)
...
abstract contract MintZilla {

    address private mzAdmin;
    
    constructor(address _mzAdmin) payable {
        mzAdmin = _mzAdmin;
        payable(_mzAdmin).transfer(msg.value);
    }

    function createdOnMintZilla() public pure returns (bool) {
        return true;
    }
}
...
contract Token is Context, IERC20, Ownable, MintZilla {
    ...
    bool public mintedByMintZilla = true;
    ...
    constructor (address _mzAdmin, string memory name_, string memory symbol_, uint8 decimal, uint256 tokenSupply, uint8 setTaxFee, uint8 setLiqFee, address router_) payable MintZilla(_mzAdmin) {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimal;
        router = router_;
        _tTotal = tokenSupply * 10 ** decimal;
